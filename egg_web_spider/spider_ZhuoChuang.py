...
from egg_web_spider.storage_data import storage_infos
...
class Egg_Web_Spider():
    def __init__(self):
        ...
        # 账号
        self.account_number = '18174504650'
        # 密码
        self.pwd = '123456'
        # 数据
        self.data = []
        self.save_path='D://CCDY_data/蛋e网企业一览'
    ...
    def slide_alignment(self, driver):
        ...
        driver.swipe(x1, y1, x1, y2)
    def slide_litte_down(self,driver):
        size = self.get_screen_size(driver)
        # slide_screen start
        x1 = int(size[0] * 0.5)
        y1 = int(size[1] * 0.5)
    ...
    def enter_egg_yellow_pages_actions(self, driver):
        ...
        if numbers != None:
            ...
            for i in range(23, numbers + 1):
                ...
                time.sleep(0.5)
                try:
                    driver.find_element_by_xpath("//android.view.View[@text='农大3号']")
                    WebDriverWait(driver, 5, 0.2).until(lambda x: x.find_element_by_xpath(
                        "//android.view.View[@resource-id='content']/android.view.View[5]/android.view.View[%d]" % i)).click()
                    time.sleep(0.2)
                    self.slide_litte_down(driver)
